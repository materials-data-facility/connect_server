...
cc_authorizer = globus_sdk.ClientCredentialsAuthorizer(
    conf_client, globus_sdk.FlowsClient.scopes.manage_flows
)

print(f"Authoirized as {globus_secrets['API_CLIENT_ID']}")
print(cc_authorizer)
flows_client = globus_sdk.FlowsClient(authorizer=cc_authorizer)

with open(f"mdf_{sys.argv[1]}_flow_config.json") as f:
    config = json.load(f)

description = "MDF Connect Flow deployed manually"
globus_auth = GlobusAuthManager(
    globus_secrets["API_CLIENT_ID"], globus_secrets["API_CLIENT_SECRET"]
)
...
mdf_flow = GlobusAutomateFlow.from_flow_def(
    client=flows_client, flow_def=flow_def, globus_auth=globus_auth
)
...
print(
    "PUT this to https://auth.globus.org/v2/api/scopes/:SCOPE_ID where SCOPE_ID is the ID of your API's scope"
)
connect_scope_def = {
    "scope": {
        "name": "MDF Connect",
